...
// ==============================================================================================================================
// 위 코드는 원래 있던 코드, 아래 코드는 1. +현재위치에서 검색되는 코드
// ==============================================================================================================================
...
function Kakaomap({ searchPlace, searchBtnToggle }) {
  ...
  useEffect(() => {
    ...
    // 현재 지도 위치
    kakao.maps.event.addListener(initialMap, 'dragend', function () {
      let center = initialMap.getCenter(); // let latlng = initialMap.getCenter();
      setLatlng([center.getLat(), center.getLng()]); // setLatlng([latlng.getLat(), latlng.getLng()]);
    });
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          const userLatLng = new kakao.maps.LatLng(latitude, longitude);
          initialMap.setCenter(userLatLng);
          // 변경전 :  handleSearch(searchPlace); 삭제 됨
        },
        (error) => {
          console.error('Error getting user location:', error);
          // 변경전 : handleSearch(searchPlace); 삭제 됨
      ...
      console.error('Geolocation is not supported by this browser.');
      // 변경전 : andleSearch(searchPlace); 삭제 됨
    }
